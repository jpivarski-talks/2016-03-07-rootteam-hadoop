...
\begin{frame}{Major tools (that I frequently encountered)}
\begin{description}
...
\item[Apache HBase] random-access tabular database on Hadoop (column-oriented key-value).
\item[Apache Hive] SQL over Hadoop ({\it not} random-access).
\item[Apache Pig] map-filter-groupby in a custom language, ``eats anything.''


\item[Apache Mesos] cluster abstraction (using ZooKeeper).
