...
Generalizes from map-reduce to arbitrary DAG data pipelines, optimized for iterative procedures, with an interactive prompt. May be used on any cluster manager, but usually Hadoop (ZooKeeper) or Apache Mesos.
\begin{itemize}
\item User interfaces: native Scala, Java, Python (through sockets), and R (through pipes).
\item Spark-Streaming (real-time), Spark-SQL, and MLLib (machine learning).
\end{itemize}
\end{block}
