...
\begin{itemize}
\item {\tt iterator<X> filter(iterator<X> input, function<bool(X)> f)}
\item {\tt iterator<Y> map(iterator<X> input, function<Y(X)> f)}
\item {\tt iterator<Y> flatMap(iterator<X> input, function<iterator<Y>(X)> f)}
\item {\tt Y reduce(iterator<Y> input, function<Y(Y,Y)> f)}
\item {\tt map<K,vector<V>> groupByKey(iterator<pair<K,V>> input)}
\item {\tt map<K,V> reduceByKey(iterator<pair<K,V>> input, function<V(V,V)> f)}
\item {\tt map<K,Z> aggregateByKey(iterator<pair<K,V>> input, Z zero, function<Z(V,Z)> seq, function<Z(Z,Z)> comb)}
\end{itemize}
