...
\begin{description}
\item[map] converts each input {\tt X} into a key value pair: {\tt pair<K,V>}.
\item[reduce] sees all values {\tt iterator<V>} with a given key {\tt K} and produces some output {\tt Z}.
\end{description}

\begin{center}
\includegraphics[width=0.7\linewidth]{map-reduce.png}
\end{center}
\end{frame}
