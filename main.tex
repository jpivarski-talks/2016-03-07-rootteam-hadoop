...
\begin{itemize}
\item {\tt iterator<X> filter(iterator<X>, function<bool(X)>)}
\item {\tt iterator<Y> map(iterator<X>, function<Y(X)>)}
\item {\tt iterator<Y> flatMap(iterator<X>, function<iterator<Y>(X)>)}
\item {\tt Y reduce(iterator<Y>, function<Y(Y,Y)>)}
\item {\tt map<K,vector<V>> groupByKey(iterator<pair<K,V>>)}
\item {\tt map<K,V> reduceByKey(iterator<pair<K,V>>, function<V(V,V)>)}
\item {\tt map<K,Z> aggregateByKey(iterator<pair<K,V>>, Z, function<Z(V,Z)>, function<Z(Z,Z)>)}
\end{itemize}
