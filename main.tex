...
\begin{itemize}
\item {\tt list<Y> map(list<X> input, Y(X))}
\item {\tt filter}
\item {\tt flatMap}
\item {\tt reduce}
\item {\tt fold}
...
\vspace{0.5 cm}
Hadoop ``mappers'' are like {\tt flatMap}, in that a single input can yield zero or more outputs, and Hadoop ``reducers'' are like {\tt aggregateByKey}, in that they can produce a different output type than they receive as input.
\end{frame}
