...
\begin{description}
\item[Apache Hadoop] framework for performing map-reduce calculations. Used as a foundation for a big data cluster because of the mind-share that developed around\ldots
\begin{itemize}
\item its distributed filesystem HDFS (even variants like MapR, which don't use HDFS, use its API).
\item its suite of InputFormats that split files by logical records.
\item its Zookeeper, which coordinates job configuration and synchronization across a distributed service.
\end{itemize}
