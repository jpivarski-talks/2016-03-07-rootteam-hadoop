...
\vspace{\baselineskip}
I never saw any interest in the GRID concept of sending jobs ``anywhere'' or ``wherever the data are.'' Even cloud computing like Amazon Web Services target specific sites, like {\tt us-east-1}, {\tt eu-west-1}, {\tt ap-northeast-1}\ldots
\end{block}
...
\begin{frame}[fragile]{Standard functional primitives in C++ type syntax}
\small
\begin{verbatim}
...
\normalsize
Hadoop's ``mappers'' are as powerful as {\tt flatMap} and ``reducers'' are {\tt aggregateByKey}.
\end{frame}
